    ...
    HOGQL_CLICKHOUSE_FUNCTIONS: dict[str, HogQLFunctionMeta] = {
        ...
        **{
        ...
        "make_timestamp": HogQLFunctionMeta(
            ...
            signatures=[
                (
                    (IntegerType(), IntegerType(), IntegerType(), IntegerType(), IntegerType(), FloatType()),
                    DateTimeType(),
                ),
                (
                    (
...
def find_hogql_function(name: str) -> Optional[HogQLFunctionMeta]:
    # Fast lookup with original case
    func = HOGQL_CLICKHOUSE_FUNCTIONS.get(name)
    if func is not None:
        return func

    # Only lowercase once if not found by original name
    lowered = name.lower()
    func = HOGQL_CLICKHOUSE_FUNCTIONS.get(lowered)
    if func is None or func.case_sensitive:
        return None
    return func
